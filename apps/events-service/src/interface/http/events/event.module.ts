...
import {
  Event,
  EventSchema,
} from '../../../infrastructure/mongoose/event.schema';
import { EventsController } from './events.controller';
import { EventsRmqController } from '../../rmq/events.rmq.controller';
...
@Module({
  ...
  controllers: [EventsController, EventsRmqController],
  providers: [
    {
    ...
    },
    {
      ...
      useFactory: (repo: any) => new CreateEventUseCase(repo),
      inject: [EVENT_REPOSITORY],
    },
    {
      provide: GetEventsUseCase,
      useFactory: (repo: any) => new GetEventsUseCase(repo),
      inject: [EVENT_REPOSITORY],
    },
    {
      provide: UpdateEventUseCase,
      useFactory: (repo: any) => new UpdateEventUseCase(repo),
      inject: [EVENT_REPOSITORY],
    },
    {
      provide: DeleteEventUseCase,
      useFactory: (repo: any) => new DeleteEventUseCase(repo),
      inject: [EVENT_REPOSITORY],
    },
  ],
})
