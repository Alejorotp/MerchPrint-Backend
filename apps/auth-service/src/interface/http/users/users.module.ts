...
import {
  User,
  UserSchema,
} from '../../../infrastructure/users/mongoose/user.schema';
...
import {
  Role,
  RoleSchema,
} from '../../../infrastructure/roles/mongoose/role.schema';
import { ExistsRoleByIdUseCase } from '../../../application/roles/usecases/exists-role.usecase';
import { UsersController } from './users.controller';
import { UsersRmqController } from '../../rmq/users.rmq.controller';
...
import {
  ExistsByEmailUseCase,
  ExistsByIdUseCase,
} from 'apps/auth-service/src/application/users/usecases/exists-user.usecase';
...
@Module({
  ...
  controllers: [UsersController, UsersRmqController],
  providers: [
    {
    ...
    {
      provide: ExistsRoleByIdUseCase,
      useFactory: (repo: any) => new ExistsRoleByIdUseCase(repo),
      inject: [ROLE_REPOSITORY],
    },
    {
      provide: CreateUserUseCase,
      useFactory: (repo: any, passwordService: any, existsRoleById: any) =>
        new CreateUserUseCase(repo, passwordService, existsRoleById),
      inject: [USER_REPOSITORY, PASSWORD_SERVICE, ExistsRoleByIdUseCase],
    },
    {
      provide: GetUserUseCase,
      useFactory: (repo: any) => new GetUserUseCase(repo),
      inject: [USER_REPOSITORY],
    },
    {
      provide: DeleteUserUseCase,
      useFactory: (repo: any) => new DeleteUserUseCase(repo),
      inject: [USER_REPOSITORY],
    },
    {
    ...
    },
    {
      ...
      useFactory: (repo: any) => new GetAllUsersUseCase(repo),
      inject: [USER_REPOSITORY],
    },
    {
      provide: ExistsByEmailUseCase,
      useFactory: (repo: any) => new ExistsByEmailUseCase(repo),
      inject: [USER_REPOSITORY],
    },
    {
      provide: ExistsByIdUseCase,
      useFactory: (repo: any) => new ExistsByIdUseCase(repo),
      inject: [USER_REPOSITORY],
    },
    {
      provide: LoginUseCase,
      useFactory: (repo: any, passwordService: any, tokenService: any) =>
        new LoginUseCase(repo, passwordService, tokenService),
      inject: [USER_REPOSITORY, PASSWORD_SERVICE, TOKEN_SERVICE],
    },
    {
      provide: RefreshTokenUseCase,
      useFactory: (repo: any, tokenService: any) =>
        new RefreshTokenUseCase(repo, tokenService),
